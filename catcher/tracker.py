...
limit = int(input('Speed Limit(kph):')) #km/hr

file = open("Traffic Record/SpeedRecord.txt","w")
file.write("ID \t SPEED\n------\t-------\n")
file.close()
...
class EuclideanDistTracker:
    ...
    def capture(self,img,x,y,h,w,sp,id):
        if(self.capf[id]==0):
            ...
            self.count += 1
            filet = open("Traffic Record/SpeedRecord.txt", "a")
            if(sp>limit):
                file2 = 'Traffic Record/exceeded/' + n + '.jpg'
    ...
    #TEXT FILE SUMMARY
    def end(self):
        file = open("Traffic Record/SpeedRecord.txt", "a")
        file.write("\n-------------\n")
        file.write("-------------\n")
        file.write("SUMMARY\n")
        file.write("-------------\n")
